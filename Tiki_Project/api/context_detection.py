...
def detectContext(product: Dict[str, Any], keyword: str) -> str:
    ...
    strong_book_indicators = title.count("sach") + title.count("book") + title.count("truyen")
    ...
    if strong_book_indicators > 0 or _contains_any(title, book_specific):
        ...
        if kw in ("xe", "oto", "o to", "xe may", "xemay", "xe dap", "xedap", "car", "motor", "bike"):
            ...
            if _contains_any(text, ["ky", "nhat ky", "hanh trinh", "du lich", "huong dan", "cam nang"]):
                return "book"
                ...
                vehicle_book_terms = ["hanh trinh", "nhat ky", "du lich", "cam nang", "huong dan", "ky su", "lich su"]
...
def getSuggestedContexts(
    ...
    counts: List[Tuple[str, int]] = [
        (ctx, len(items)) for ctx, items in grouped.items() 
