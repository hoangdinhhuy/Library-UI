        ...
        top_3_data = [
            {
                ...
                "title": p['title'],
                "price_vnd": p['price'],
                "price_vs_avg_pct": round((p['price'] - avg_price) / avg_price * 100, 1) if avg_price else 0,
                "rating": p['rating'],
...
"""
    ...
    def _get_detailed_ml_insights(self, products: List[Dict]) -> str:
        ...
        if self.model_loader.kmeans_model and self.data_loader.products_df is not None:
            try:
                ...
                if cluster_info:
                    ...
                    for cname, count in sorted(cluster_info.items(), key=lambda x: -x[1]):
        ...
        if prophet_forecasts:
            try:
                for p in products[:10]:
                    ...
                    if current_price > 0:
                        total_change += ((future_price - current_price) / current_price) * 100
                        forecasted += 1
                used_source = "Prophet"
            except Exception as e:
        ...
        if forecasted == 0 and ts_df is not None and not ts_df.empty:
            try:
                for p in products[:10]:
                    ...
                    prior  = rows['y'].iloc[-14:-7].mean() if len(rows) >= 14 else rows['y'].iloc[:-7].mean()
                    if prior > 0:
        ...
        if forecasted > 0:
            ...
            source_label = f"dự báo {used_source}" if used_source == "Prophet" else "xu hướng lịch sử"
            insights.append(f"\n**📈 Phân tích giá ({source_label}):**")
            insights.append(f"- Có dữ liệu cho {forecasted}/{len(products[:10])} sản phẩm")
            if avg_change > 5:
                insights.append(f"- Xu hướng: Giá dự kiến TĂNG {avg_change:.1f}% ⬆️")
            elif avg_change < -5:
                insights.append(f"- Xu hướng: Giá dự kiến GIẢM {abs(avg_change):.1f}% ⬇️")
            else:
                insights.append(f"- Xu hướng: Giá ổn định, biến động nhẹ ({avg_change:+.1f}%) ➡️")

        # --- 3. Sentiment từ reviews.json (PhoBERT pre-labeled) ---
        rv_df = getattr(self.data_loader, 'reviews_df', None)
        if self.model_loader.phobert_available and rv_df is not None and not rv_df.empty:
            try:
                pid_list = []
                for p in products[:10]:
                    try:
    ...
    def _generate_fallback_insight(
        ...
        def _desc(p):
            price_diff = (p['price'] - avg_price) / avg_price * 100 if avg_price > 0 else 0
            if price_diff < -15:
                price_label = f"giá thấp hơn trung bình thị trường {abs(price_diff):.0f}%"
            elif price_diff > 15:
            ...
            if rating_diff >= 0.3:
                ...
                quality_label = f"rating dưới chuẩn ({p['rating']:.1f}⭐), cần cải thiện"
            else:
        ...
        top_cat_name, top_cat_count = max(cats.items(), key=lambda x: x[1]) if cats else ("N/A", 0)
        top_cat_pct = top_cat_count / len(products) * 100 if products else 0

        min_price = min(p['price'] for p in products)
        max_price = max(p['price'] for p in products)

        seg_counts = {"bình dân": len(budget_products), "trung cấp": len(mid_products), "cao cấp": len(premium_products)}
...
"""
    ...
    def generate_market_report(self, keyword: str, products: List[Dict]) -> Dict:
        ...
        avg_rating = sum(p['rating'] for p in products) / len(products)
        total_sold = sum(p['boughtInLastMonth'] for p in products)
        total_revenue = sum(p.get('estimated_revenue', p['price'] * p['boughtInLastMonth']) for p in products)
        max_price = max(p['price'] for p in products)
        min_price = min(p['price'] for p in products)
        ...
        for i, p in enumerate(products[:10], 1):
            top_products.append({
                ...
                "price": p['price'],
                "sold": p['boughtInLastMonth'],
                "revenue": p.get('estimated_revenue', p['price'] * p['boughtInLastMonth']),
                "rating": p['rating'],
                "category": p['categoryName'],
        ...
        sentiment = {"total": 0, "positive": 0, "neutral": 0, "negative": 0, "pos_pct": 0, "neu_pct": 0, "neg_pct": 0}
        rv_df = getattr(self.data_loader, 'reviews_df', None)
            ...
            try:
                ...
                if pid_list:
                    ...
                    if total_rv > 0:
                        ...
                        sentiment = {
                            "total": total_rv, "positive": pos, "neutral": neu, "negative": neg,
        ...
        price_trend = {"direction": "stable", "avg_change_pct": 0.0, "source": "N/A", "products_analyzed": 0}
        prophet_forecasts = getattr(self.model_loader, 'prophet_forecasts', None)
        ts_df = getattr(self.data_loader, 'timeseries_df', None)
        forecasted = 0
        total_change = 0.0
        ...
        if forecasted > 0:
            avg_change = total_change / forecasted
            direction = "up" if avg_change > 5 else ("down" if avg_change < -5 else "stable")
            price_trend = {
                "direction": direction,
            ...
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    if cname not in cluster_map:
                        cluster_map[cname] = {"count": 0, "price_sum": 0.0, "rating_sum": 0.0}
                    cluster_map[cname]["count"] += 1
                    cluster_map[cname]["price_sum"] += float(row.get('original_price', 0))
                    cluster_map[cname]["rating_sum"] += float(row.get('original_rating', 0))
                for cname, data in sorted(cluster_map.items(), key=lambda x: -x[1]["count"]):
                    cnt = data["count"]
                    clusters.append({
                        "name": cname,
                        "count": cnt,
                        "avg_price": round(data["price_sum"] / cnt, 0) if cnt > 0 else 0,
                        "avg_rating": round(data["rating_sum"] / cnt, 2) if cnt > 0 else 0,
                    })
            except Exception as e:
                logger.debug(f"Cluster report failed: {e}")

        # --- AI Report ---
        ai_report = self.generate_insight(products=products, keyword=keyword, include_ml_insights=True)
        ...
        return {
            ...
            "price_segments": {
                "budget": {"count": len(budget), "pct": round(len(budget) / len(products) * 100, 1)},
                "mid":    {"count": len(mid),    "pct": round(len(mid)    / len(products) * 100, 1)},
                "premium":{"count": len(premium),"pct": round(len(premium)/ len(products) * 100, 1)},
            },
