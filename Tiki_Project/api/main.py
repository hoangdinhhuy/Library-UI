...
async def market_insight(category: Optional[str] = None):
    ...
    try:
        ...
        if category and category.strip():
            df = df[df['category'] == category.strip()]
            if df.empty:
        ...
        if df.empty:
            return {
                ...
                "data": {
                    ...
                    "n_clusters": 5,
                    "blue_ocean_count": 0,
                    "silhouette_score": round(float(model_loader.kmeans_metadata.get("metrics", {}).get("silhouette_score", 0.464)), 4) if hasattr(model_loader, "kmeans_metadata") else 0.464,
                    "max_popularity": 0.0,
        ...
        market_avg_rating = float(df['effective_rating'].mean()) if not df.empty else 0.0
        rating_threshold = min(4.2, max(2.5, market_avg_rating - 0.4))
        pop_threshold = float(df['popularity_score'].quantile(0.6)) if 'popularity_score' in df.columns else 0.0
        avg_price_threshold = float(df['original_price'].mean()) if not df.empty else 0.0

        for cid in sorted(df['cluster_id'].unique()):
            if cid < 0:
                continue
            sub = df[df['cluster_id'] == cid]
            avg_rating_orig = float(sub['effective_rating'].mean())
            ...
            cluster_summary.append({
                ...
                'top_category': str(sub['category'].mode().iloc[0]) if not sub.empty else '',
                    'is_blue_ocean': avg_rating_orig <= rating_threshold and avg_pop >= pop_threshold,
                'top_products': top_products,
            })

        qty_q95 = float(df['quantity_sold'].quantile(0.95)) if 'quantity_sold' in df.columns else 1.0
        ...
        for _, row in blue_df.iterrows():
            ...
            blue_ocean_products.append({
                ...
                'rating': float(row['effective_rating']), 'quantity_sold': int(row['quantity_sold']),
                'popularity_score': round(float(row['popularity_score']), 3),
                'opportunity_score': round(float(row['product_opportunity_score']), 1),
        ...
        for _, row in df.iterrows():
            ...
            all_products_out.append({
                ...
                'category': str(row['category']), 'price': float(row['original_price']),
                'rating': float(row['effective_rating']), 'quantity_sold': int(row['quantity_sold']),
                'popularity_score': round(float(row['popularity_score']), 3) if 'popularity_score' in row else 0.0,
                'opportunity_score': round(float(row['product_opportunity_score']), 1),
                'cluster_name': str(row['cluster_name']), 'url': url,
            })
        ...
        return {'success': True, 'data': {
            ...
            'rating_threshold': round(rating_threshold, 2),
            'market_avg_rating': round(market_avg_rating, 2),
            'pop_threshold': round(pop_threshold, 3),
            'avg_price_threshold': round(avg_price_threshold, 2),
        }}
    except Exception as e:
...
async def get_categories():
    ...
    try:
        cats = sorted(data_loader.products_df['category'].dropna().unique().tolist())
