...
const DEFAULT_DATA = {
  api: {
    baseUrl: "https://your-databricks-app-url.com",
    endpoints: {
      single: "/api/search",
  ...
  mockProductsSingle: [
    { id: 1, name: "Tai nghe Bluetooth TWS Pro", cat: "Điện tử", price: "350k", sold: "2.450", rev: "857M" },
    { id: 2, name: "Áo Thun Basic Unisex", cat: "Thời trang", price: "129k", sold: "3.100", rev: "399M" }
  ],
  mockProductsBatch: [
    { id: 1, name: "Dữ liệu từ File CSV 1", cat: "Tổng hợp", price: "500k", sold: "100", rev: "50M" },
    { id: 2, name: "Dữ liệu từ File CSV 2", cat: "Tổng hợp", price: "200k", sold: "500", rev: "100M" },
    { id: 3, name: "Dữ liệu từ File CSV 3", cat: "Tổng hợp", price: "150k", sold: "800", rev: "120M" }
  ]
};
...
function App({ appData }) {
  ...
  const executeAnalysis = async (type, payload) => {
    ...
    try {
      const API_URL = appData.api.baseUrl;

      let response;
      if (type === "single") {
        response = await fetch(`${API_URL}${appData.api.endpoints.single}`, {
          method: "POST",
        ...
        response = await fetch(`${API_URL}${appData.api.endpoints.batch}`, {
          method: "POST",
          body: formData
        });
      }
      ...
      if (type === "single") {
        ...
        setInsightSingle(result.data.ai_insight);
      } else {
        setResultBatch(
        ...
        setInsightBatch(result.data.ai_insight);
      }
  ...
  const handleSingleExecute = () => {
    ...
    executeAnalysis("single", { keyword });
  };

  const handleBatchExecute = () => {
    if (!selectedFile) return;
    executeAnalysis("batch", { file: selectedFile });
  };
  ...
  const renderResult = (result, insight) => (
    <div ref={reportRef} className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-10 mt-8">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: "Tổng Doanh Thu", val: appData.kpi.revenue, icon: "dollar-sign", color: "text-green-400" },
          { label: "Sản Phẩm Bán Ra", val: appData.kpi.sold, icon: "shopping-bag", color: "text-blue-400" },
          { label: "Giá TB Đơn Hàng", val: appData.kpi.avg, icon: "tag", color: "text-purple-400" },
          { label: "Tăng Trưởng", val: appData.kpi.growth, icon: "trending-up", color: "text-rose-400" }
        ].map((item, idx) => (
  ...
  return (
    <div className="flex h-screen bg-[#0f172a] overflow-hidden">
      <div className="w-64 bg-[#1e293b] border-r border-gray-700 flex flex-col z-20 shadow-xl">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${
              activeTab === "single" ? "bg-rose-600 text-white shadow-md" : "text-gray-400 hover:bg-gray-800 hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${
              activeTab === "batch" ? "bg-rose-600 text-white shadow-md" : "text-gray-400 hover:bg-gray-800 hover:text-white"
            }`}
          >
        ...
        <div className="p-4 border-t border-gray-700 text-xs text-gray-500 text-center">v1.0.0 • Deployed on Digital Ocean</div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-16 bg-[#1e293b] border-b border-gray-700 flex items-center justify-between px-8 z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center text-white font-bold border border-gray-600">A</div>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto p-8 scroll-smooth">
            ...
            <div className="max-w-4xl mx-auto mb-8">
              <div className="glass-panel p-8 rounded-xl text-center animate-fade-in">
                ...
                >
                  ...
                  <h3 className="text-lg font-bold text-white mb-1">{selectedFile ? selectedFile.name : "Click để tải lên tệp CSV"}</h3>
          ...
          {!loadingSingle && resultSingle && activeTab === "single" && renderResult(resultSingle, insightSingle)}
          {!loadingBatch && resultBatch && activeTab === "batch" && renderResult(resultBatch, insightBatch)}
          ...
          {!loadingSingle && !resultSingle && !loadingBatch && !resultBatch && (
            <div className="flex flex-col items-center justify-center h-64 text-gray-500 opacity-60">
              ...
              <p className="text-lg">
                {activeTab === "single" ? "Nhập từ khóa để bắt đầu phân tích thị trường" : "Tải lên file CSV để phân tích dữ liệu hàng loạt"}
              </p>
            </div>
          )}
        </main>
  ...
  try {
    ...
    return {
      ...
      api: {
        ...
        ...(fileData.api || {}),
        endpoints: {
          ...DEFAULT_DATA.api.endpoints,
          ...((fileData.api || {}).endpoints || {})
        }
      },
      kpi: {
        ...DEFAULT_DATA.kpi,
    ...
    console.warn("Fallback to default data because data.json is unavailable.", err);
    return DEFAULT_DATA;
  }
}
...
(async function bootstrap() {
  ...
  root.render(<App appData={appData} />);
})();
